    ...
    <div class="content-box">
        ...
        <div class="content-box-content">
            <form class="search-content" enctype="multipart/form-data" method="POST"
                ...
                <table>
                    <tbody>
                        <input type="hidden" name="article_id" value="<?php echo $search['article_id'];?>">
                        <tr>
                            <td>文章标题：
                            ...
                            <td>
                                ...
                                <img src="<?php echo $search['cover_photo'];?>" id="cover" style="width:40px; height:40px; margin-left:150px;margin-top:-40px;" />
                            </td>
                        </tr>
                        <tr>
                            <td>文章分类：
...
<script type="text/javascript">
    ...
    var add = function () {
        ...
        $('input[name=add_submit]').attr('disabled', true).removeClass('submit').val('正在修改中...');
        $.post('<?php echo HOME_DOMAIN; ?>adm_article/edit_article', $('form').serialize(), function (response) {
            if (true === response.success) {
                layer.alert('文章修改成功', {icon: 1}, function(){
                    $.BKD.refresh();
                });
            } else {
                layer.alert(response.message, {icon: 2});
                $('input[name=add_submit]').attr('disabled', false).addClass('submit').val('修   改');
            }
        }, 'JSON');
    };
</script>