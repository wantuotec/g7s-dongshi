 ...
 * @version     $Id:$
 */
class S_website_notice_model extends CI_Model
{
    // 不定义默认和model保持一致
    public $id  = 'website_notice_id';
    public $dao = 'operation/s_website_notice_dao';
    ...
    public function update_by_params($params = [])
    {
        ...
        // 按mood_id进行修改
        if (isset($params['website_notice_id']) && !empty($params['website_notice_id'])) {
            $result = $this->update_by_id($params['website_notice_id'], $params['set']);
        // 都不存在时，给出提示
        } else {
