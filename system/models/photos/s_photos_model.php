 ...
 * @version     $Id:$
 */
class S_photos_model extends CI_Model
{
    // 不定义默认和model保持一致
    public $id  = 'photos_id';
    public $dao = 'photos/s_photos_dao';
    ...
    public function update_by_params($params = [])
    {
        ...
        // 按photos_id进行修改
        if (isset($params['photos_id']) && !empty($params['photos_id'])) {
            $result = $this->update_by_id($params['photos_id'], $params['set']);
        // 都不存在时，给出提示
        } else {
